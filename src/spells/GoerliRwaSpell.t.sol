...
contract DssSpellTest is DSTest, DSMath {
    ...
    bytes32 constant ilk = "RWA007AT1-A";
    DSTokenAbstract rwagem = DSTokenAbstract(addr.addr("RWA007AT1"));
    GemJoinAbstract rwajoin = GemJoinAbstract(addr.addr("MCD_JOIN_RWA007AT1_A"));
    RwaLiquidationLike oracle = RwaLiquidationLike(addr.addr("MIP21_LIQUIDATION_ORACLE_2"));
    RwaUrn2Like rwaurn = RwaUrn2Like(addr.addr("RWA007AT1_A_URN"));
    RwaInputConduitLike rwaconduitin = RwaInputConduitLike(addr.addr("RWA007AT1_A_INPUT_CONDUIT"));
    RwaOutputConduitLike rwaconduitout = RwaOutputConduitLike(addr.addr("RWA007AT1_A_OUTPUT_CONDUIT"));
    ...
    function setUp() public {
        ...
        //
        afterSpell.collaterals["RWA007AT1-A"] = CollateralValues({ // TODO
    ...
    function testChainlogValues() public {
        ...
        assertEq(chainlog.getAddress("RWA007AT1"), addr.addr("RWA007AT1"));
        assertEq(chainlog.getAddress("MCD_JOIN_RWA007AT1_A"), addr.addr("MCD_JOIN_RWA007AT1_A"));
        assertEq(chainlog.getAddress("RWA007AT1_A_URN"), addr.addr("RWA007AT1_A_URN"));
        assertEq(chainlog.getAddress("RWA007AT1_A_INPUT_CONDUIT"), addr.addr("RWA007AT1_A_INPUT_CONDUIT"));
        assertEq(chainlog.getAddress("RWA007AT1_A_OUTPUT_CONDUIT"), addr.addr("RWA007AT1_A_OUTPUT_CONDUIT"));
        assertEq(chainlog.getAddress("MIP21_LIQUIDATION_ORACLE_2"), addr.addr("MIP21_LIQUIDATION_ORACLE_2"));
    }

    function testSpellIsCast_RWA007AT1_INTEGRATION_BUMP() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast();
            assertTrue(spell.done());
    ...
    function testSpellIsCast_RWA007AT1_INTEGRATION_TELL() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast();
            assertTrue(spell.done());
        ...
        uint256 castTime = block.timestamp + pause.delay();
        hevm.warp(castTime);
        (, , , uint48 tocPre) = oracle.ilks("RWA007AT1-A");
        assertTrue(tocPre == 0);
        assertTrue(oracle.good("RWA007AT1-A"));
        tellSpell.cast();
        (, , , uint48 tocPost) = oracle.ilks("RWA007AT1-A");
    ...
    function testSpellIsCast_RWA007AT1_INTEGRATION_TELL_CURE_GOOD() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast();
            assertTrue(spell.done());
    ...
    function testFailSpellIsCast_RWA007AT1_INTEGRATION_CURE() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast();
            assertTrue(spell.done());
    ...
    function testSpellIsCast_RWA007AT1_INTEGRATION_TELL_CULL() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast();
            assertTrue(spell.done());
    ...
    function testSpellIsCast_RWA007AT1_OPERATOR_LOCK_DRAW_CONDUITS_WIPE_FREE() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast();
            assertTrue(spell.done());
        ...
        (, uint256 rate, , , ) = vat.ilks("RWA007AT1-A");
        ...
        jug.drip("RWA007AT1-A");

        (, rate, , , ) = vat.ilks("RWA007AT1-A");
    ...
    function testFailSpellIsCast_RWA007AT1_OPERATOR_LOCK_ABOVE_CAP() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast();
            assertTrue(spell.done());
    ...
    function testSpellIsCast_RWA007AT1_URN_ADMIN_CAN_INCREASE_GEM_CAP() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast();
            assertTrue(spell.done());
    ...
    function testFailSpellIsCast_RWA007AT1_URN_FAIL_ON_NOT_OPERATOR_INCREASE_GEM_CAP() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast();
            assertTrue(spell.done());
    ...
    function testFailSpellIsCast_RWA007AT1_URN_FAIL_TO_LOCK_GEM_BEYOND_CAP_AFTER_DECREASE_GEM_CAP() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast();
            assertTrue(spell.done());
    ...
    function testSpellIsCast_RWA007AT1_END() public {
        if (!spell.done()) {
            vote(address(spell));
            scheduleWaitAndCast();
            assertTrue(spell.done());
